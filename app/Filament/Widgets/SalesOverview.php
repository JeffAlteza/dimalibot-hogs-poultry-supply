...
class SalesOverview extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $totalProfit = Sales::where('status', 'Paid')->whereMonth('updated_at', Carbon::now()->month)->sum('profit');

        $totalProfitThisMonth = $sales->where('status', 'Paid')->sum('profit');
        ...
        return [
            Stat::make('Sales', '₱' . number_format($totalSales, 2))
                ->description('Total Sales')
                ->descriptionIcon('heroicon-o-banknotes')
                ->color('primary'),
            Stat::make('Profit', '₱' . number_format($totalProfit, 2))
                ->description('Total Profit')
                ->descriptionIcon('heroicon-o-banknotes')
                ->color('primary'),
            Stat::make('Profit this ' . Carbon::now()->format('F'), '₱' . number_format($totalProfitThisMonth, 2))
                ->description('Total Profit')
                ->descriptionIcon('heroicon-o-banknotes')
                ->color('primary'),
